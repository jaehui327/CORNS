...
function FriendsBtn({ fromId, toId, status, setRelation }) {
  switch (status) {
    ...
    case 0:
      return (
        <FriendBtnZero fromId={fromId} toId={toId} setRelation={setRelation} />
      );
    ...
    case 2:
      if (!window.location.pathname.includes("friend")) {
        return (
          <FriendBtnTwo fromId={fromId} toId={toId} setRelation={setRelation} />
        );
      } else {
        return <></>;
      }
    ...
    case 3:
      return (
        <FriendBtnThree fromId={fromId} toId={toId} setRelation={setRelation} />
      );
